...
class DQNAgent():
    def __init__(self):
        # 클래스의 함수들을 위한 값 설정 
        self.model = model.DQN_net(config.action_size, "main").to(device)
        self.target_model = model.DQN_net(config.action_size, "target").to(device)
    ...
    def get_action(self, state):
        if self.epsilon > np.random.rand():
            ...
            # 네트워크 연산에 따라 행동 결정
            Q = self.model(torch.from_numpy(state).unsqueeze(0).to(device))
    ...
    def train_model(self, done):
        ...
        # 타겟값 계산
        predict_Q = self.model(torch.FloatTensor(state_batch).to(device)) 
        target_Q = predict_Q.cpu().detach().numpy()
        target_nextQ = self.target_model(torch.FloatTensor(next_state_batch).to(device)).cpu().detach().numpy()
        ...
        loss = F.smooth_l1_loss(predict_Q.to(device), torch.from_numpy(target_Q).to(device))
